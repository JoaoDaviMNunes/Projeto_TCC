...
with open('pesosReports.csv') as p, open ('contagemPesos.csv', 'w', newline='') as c:
	...
	for linha in pesos:
		# ignora a 1a linha, que descreve o que são as colunas (arquivo de entrada)
		if totalLinhas != 0:
			#verifica se a precisão foi de 95% e soma no contador
			if float(linha[6]) >= 95.0:
				totalPrecisao += 1
				match int(linha[0]):
					case 0:
						pes00 += 1
	...
	print("Total de " + str(totalPrecisao) + " linhas que satisfizeram a precisão.")
	print(round(totalPrecisao/(totalLinhas-1)*100,2))
